...
export namespace Example {
  ...
  export function run(sources, args: Arguments): { DOM: Stream<VNode> } {
    function main(sources) {
      ...
      let transition$ = btnShow.events("click").fold((prev, n) => !prev, false).drop(1).map(
        n => ({
      ...
      //Example bar
      let top$ = xs.combine(transition$, args.VNode$).map(([animation, obj]) => {
