...
describe("Icon", function () {
  describe("render", function () {
    ...
    it("should support argument syntax", function () {
      ...
      assert.equal(icon.data.props.className, "circular close icon");
      icon = Icon.render("close");
      assert.equal(icon.data.props.className, "close icon");
      icon = Icon.render({ circular: true });
      assert.equal(icon.data.props.className, "circular icon");
    });
    ...
    it("should support the button variation", function () {
      ...
      assert.equal(icon.data.props.className, "button icon");
    });
    it("should support the bordered variation", function () {
      let icon = Icon.render({ bordered: true });
      assert.equal(icon.data.props.className, "bordered icon");
    });
  ...
  describe("run", function () {
    ...
    it("should return a icon stream", function (done) {
      ...
      icon.DOM.addListener({
        next: (x) => {
          assert.equal("close icon", x.data.props.className);
          done();
        }
      });
    });
